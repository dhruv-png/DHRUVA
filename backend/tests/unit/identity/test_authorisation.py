...
def test_a_permission_is_granted_only_by_being_named() -> None:
    ...
    trader = Role(name="trader", granted=frozenset({Permission.PLACE_ORDER}))
...
def test_a_role_granted_every_other_permission_still_cannot_place_an_order() -> None:
    ...
    one.
    """
    everything_else = Role(
        name="administrator",
        granted=frozenset(Permission) - {Permission.PLACE_ORDER},
    )

    assert not is_permitted(everything_else, Permission.PLACE_ORDER)


def test_the_permission_set_is_exactly_what_the_plan_names() -> None:
    """Pinned. Plan §15.1 names order placement and nothing else.
    ...
    """
    assert sorted(permission.value for permission in Permission) == ["place_order"]
