"""Market-data transaction boundary with account-scoped session context."""
...
__all__ = ["SqlAlchemyMarketDataUnitOfWork"]

_SESSION_ACCOUNT_SETTING = "dhruva.current_account_id"


class SqlAlchemyMarketDataUnitOfWork:
    """One caller-owned transaction exposing only daily market-data facts."""
    ...
    @property
    def daily_bars(self) -> DailyBarRepository:
        """Return the daily-bar repository bound to this transaction."""
    ...
    async def commit(self) -> None:
        """Commit every staged daily bar revision."""
        await self.session.commit()
        self._committed = True

    async def rollback(self) -> None:
        """Discard every staged daily bar revision."""
